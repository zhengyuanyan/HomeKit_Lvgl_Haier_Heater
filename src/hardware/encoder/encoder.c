...
static const char *TAG = "Ec35_Encoder";
...
#define EC35_PCNT_DEFAULT_HIGH_LIMIT (10000)
#define EC35_PCNT_DEFAULT_LOW_LIMIT (-10000)
...
static esp_err_t ec11_set_glitch_filter(rotary_encoder_t *encoder, uint32_t max_glitch_us)
{
    ...
    uint32_t val = max_glitch_us * 80;

    // clamp 到硬件范围
    if (val > 1023)
        val = 1023;
    if (val < 1 && max_glitch_us > 0)
        val = 1;

    return pcnt_set_filter_value(ec11->pcnt_unit, val);
}
...
static void ec11_pcnt_overflow_handler(void *arg)
{
    ...
    {
        ec11->accumu_count += EC35_PCNT_DEFAULT_HIGH_LIMIT;
    }
    else if (status & PCNT_EVT_L_LIM)
    {
        ec11->accumu_count += EC35_PCNT_DEFAULT_LOW_LIMIT;
    }
}

esp_err_t rotary_encoder_new_ec35(const rotary_encoder_config_t *config, rotary_encoder_t **ret_encoder)
{
    ...
    pcnt_config_t dev_config = {
        ...
        .unit = ec11->pcnt_unit,
        .pos_mode = PCNT_COUNT_DEC, // PCNT_COUNT_INC
        .neg_mode = PCNT_COUNT_INC, // PCNT_COUNT_DEC
        .lctrl_mode = PCNT_MODE_REVERSE,
        .hctrl_mode = PCNT_MODE_KEEP,
        .counter_h_lim = EC35_PCNT_DEFAULT_HIGH_LIMIT,
        .counter_l_lim = EC35_PCNT_DEFAULT_LOW_LIMIT,
    };
